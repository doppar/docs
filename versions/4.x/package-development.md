...
Packages are typically integrated into a Doppar application through launchers, which act as the entry point for registration and bootstrapping. A launcher handles tasks like binding services to the container, publishing resources, and registering event listeners or middleware.
...
The launcher is the core class responsible for registering and bootstrapping your package’s functionality into a Doppar application. It tells Doppar how to load your package, and it acts as the central point to hook into the application's lifecycle.

Here's a breakdown of what goes into a typical launcher and its responsibilities:
```php
<?php
...
class PackageLauncher extends ServiceLauncher
{
    /**
...
```
## Register Launcher
To integrate your package into a Doppar application, you need to register your package's launcher. The launcher is the entry point that tells Doppar how to load your package's components such as routes, views, config, and migrations.

Open your application's configuration file `runtime/config/app.php` and add your launcher with the `launchers` array
```php
'launchers' => [
    // Other launchers...
    Vendor\PackageName\PackageLauncher::class,
],
```
Replace `Vendor\PackageName\PackageLauncher` with the actual namespace and class name of your launcher.

## Configuration Files
Packages in Doppar can include their own configuration files to allow developers to customize behavior without modifying package code. These configuration files should be placed inside a config directory within your package `(e.g., runtime/config/flarion.php)`.

To make your package's configuration usable and customizable, follow these two steps:
## Merge Default Configuration
Inside your launcher’s `register()` method, use the `mergeConfig()` method to combine the package's default config with the user's application config. This ensures that default values are available while allowing overrides.
```php
public function register()
{
...
You should store your views inside a `views/` directory within your package structure. Then, register them using the loadViews() method inside your launcher’s `launch()` method:
```php
public function launch()
{
...
## Loading Migrations
You should place all migration files in a `schema/migrations/` directory within your package. Then register them in your launcher:
```php
public function launch()
{
    $this->loadMigrations(__DIR__ . '/schema/migrations');
}
```
...
Add the following to your launcher's `launch()` method:
```php
public function launch()
{
   $this->publishes([
...
If you would like to publish your package's language files to the application's `lang/vendor` directory, you may use the launcher's publishes method. The publishes method accepts an array of package paths and their desired publish locations. For example, to publish the language files for the `flarion` package, you may do the following:
```php
...
```
To publish the language files to the main application `templates/lang` directory, run:
```bash
php pool vendor:publish --tag=translations
```

## Public Assets
Your package may have assets such as JavaScript, CSS, and images. To publish these assets to the application's public directory, use the launcher's publishes method. In this example, we will also add a public asset group tag, which may be used to easily publish groups of related assets:
```php
public function launch(): void
{
    $this->publishes([
...
## Register Commands
Your package may include custom pool commands. To register them, you can use the commands method in your launcher's `launch` method. Doppar provides several ways to do this:
```php
public function launch(): void
{
...
This command will publish any templates (like views, configurations, migrations, etc.) defined within the `PackageLauncher` to the appropriate locations within your application (like the `config`, `templates/views`, or `schema/migrations` directories).