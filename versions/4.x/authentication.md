...
## Authentication
### Introduction
Authentication is a fundamental part of building secure web applications. It provides a reliable way to identify users, manage authenticated sessions, and control access to protected areas of your application. Doppar provides a complete authentication system that simplifies these common requirements while giving developers the flexibility to customize authentication flows to fit their application.

Doppar includes built-in authentication scaffolding that can generate the essential routes, controllers, views, and middleware needed to get started with user authentication. It also provides a convenient Auth facade and auth() helper for handling common authentication operations such as verifying credentials, logging users in and out, retrieving the currently authenticated user, and managing persistent sessions with "Remember Me".

Authentication can be configured around one or multiple user types through Doppar's actor-based authentication system. Multiple actors allow applications to independently authenticate different types of users, such as regular users and administrators, while keeping their sessions, cookies, and authentication state isolated from one another.

For applications that require an additional layer of account protection, Doppar also provides built-in Two-Factor Authentication (2FA) support based on TOTP. This allows you to integrate authenticator apps, recovery codes, QR-code provisioning, and verification into your existing authentication workflow.
...
## Model Configuration
The `runtime/config/auth.php` file in Doppar defines the actors and Entity models used for authentication.
...
### Configuring Actors
Open `runtime/config/auth.php` and declare each actor under the `actors` key. Every actor requires a `model` (the Entity class) and a `session_key` (a unique string used to store the user's ID in the session).
```php
<?php
