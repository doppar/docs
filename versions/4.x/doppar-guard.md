...
Doppar does not support package auto-discovery, so you need to manually register the service launcher to enable the `Guard` facade. In your `runtime/config/app.php`, add the following to the launchers array:

```php
'launchers' => [
    // Other launchers...
    \Doppar\Authorizer\GuardLauncher::class,
],
```
...
The `ComplianceService::verify` call does not happen at launch, at launcher registration, or at any point before `Guard::allows('compliance-check')` is called. If this request never reaches that check, the service is never contacted.
