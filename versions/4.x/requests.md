...
Doppar’s `Phaseolies\Http\Request` class is the central interface for working with data coming into your application. Rather than treating an HTTP request as a collection of raw values, it provides an expressive, object-oriented API for retrieving, transforming, validating, and shaping that data as it moves through your application.

From simple input access to more advanced request processing, the Request class lets you work with query parameters, headers, cookies, uploaded files, sessions, routes, and validation results through a consistent API. Its fluent transformation methods make it possible to normalize or reshape input directly at the request layer, while features such as conditional processing, structured extraction, blank-value normalization, and DTO binding help turn incoming payloads into application-ready data.

This approach keeps request handling close to where the data enters your application, reducing repetitive preparation code inside controllers. Whether you are processing a form submission, handling an API payload, or binding incoming data to a DTO, Doppar’s Request class gives you the tools to move from raw HTTP input to clean, structured application data with minimal boilerplate.
...
What ever you are doing using `Request` instance, you can do the same job using `request()` helper any where without injecting `Request` as method params. The `request()` helper is available globally, so you can use it anywhere within your Doppar application
...
You can bind the request body directly into controller parameters using the `#[BindPayload]` attribute. The router instantiates the parameter type and hydrates it via `Request::bindTo()` before your method runs.
...
To validate a DTO using constraints declared on its properties, enable `validate`:
...
With `validate: true`, the payload is checked before the controller runs and the DTO is hydrated only when validation succeeds. See [Attribute-Based DTO Validation](/versions/4.x/validation#attribute-based-dto-validation) for the available constraints, custom messages, and error responses.
