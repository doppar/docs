...
### Register Launcher
Next, register the Insight launcher so Doppar can bootstrap the profiler. Open your `runtime/config/app.php` file and add the launcher to the `launchers` array.
...
```bash
php pool vendor:publish --launcher="Doppar\Insight\ProfilerLauncher"
```
...
Here is a safer production-style example for `runtime/config/insight.php`.
...
## Data Storage
Insight stores recent request snapshots as JSON files under `storage/framework/profiler`. The `retention_days` value in `runtime/config/insight.php` controls how long those snapshots are kept before old data is removed automatically.
