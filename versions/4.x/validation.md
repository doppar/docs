...
The API Presenter provides a structured and consistent way to transform application data into clean, predictable API responses. Rather than exposing your Entity models or raw arrays directly from controllers, Presenters give you a dedicated layer for defining how your data should be represented to API consumers.

A Presenter acts as a presentation layer between your application’s data and its JSON response. It allows you to control which attributes are exposed, transform and format values, conditionally include additional data, and organize related resources without placing presentation logic inside your controllers. This keeps your controllers focused on application flow while making API responses easier to maintain and evolve.
...
Doppar lets you declare validation constraints directly on DTO properties with PHP attributes. This keeps the expected shape of a request payload next to the object that receives it, while `#[BindPayload]` can validate and hydrate the DTO before the controller method is called.

### Defining a validated DTO

The following DTO describes a book payload. `NotBlank` is used for required values, while the other attributes check the value type or range:
...
### Validating a controller payload

Set `validate: true` on `#[BindPayload]` to enable attribute validation. The router creates the DTO, validates the incoming payload, normalizes supported scalar values, and injects the hydrated object into the controller:
...
| Attribute | Arguments | Behaviour |
| --- | --- | --- |
| `#[NotBlank]` | Optional message key | Fails when the value is missing (`null`) or an empty string. |
| `#[StringType]` | Optional message key | Requires a non-null string. |
| `#[Integer]` | Optional message key | Accepts an integer or an integer string. |
| `#[Length]` | `min`, `max` (optional) | Checks the character length of a string. |
| `#[Between]` | `min`, `max` | Requires a numeric value within the inclusive range. |

Most constraints ignore `null`; combine them with `#[NotBlank]` when a property is required. `Length` also ignores non-string values, so use it with `#[StringType]` when both type and length must be enforced. `Length` uses `mb_strlen()`, making it suitable for multibyte strings.
