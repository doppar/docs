...
- Configuration files (`runtime/config/*.php`) have changed.
...
## Debug Mode
The debug option in your `runtime/config/app.php` configuration file controls the level of error information displayed to the user. By default, this option respects the value set in the `APP_DEBUG` environment variable, which is stored in your application's `env.toml` file.
...
To prevent this, ensure that the following line is set in your env.toml file for production:
```toml
APP_DEBUG = false
```

Write `false` as a real TOML boolean, not a quoted string — `APP_DEBUG = "false"` is a non-empty string, and PHP treats any non-empty string as truthy, which would leave debug mode on.
