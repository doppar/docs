...
Logging is configured in `runtime/config/log.php`. This file defines the default
channel, the available channels, and the file or remote destination each
channel should use.

Set the default channel in your `env.toml` file:
...
Each channel must exist in `runtime/config/log.php`.
