...
Doppar’s Temporal Time-Travel ORM brings automatic history tracking and time-travel capabilities directly to your models. By marking a model with the `#[Temporal]` attribute, Doppar automatically records a complete snapshot of the model whenever a record is created, updated, or deleted. These snapshots are stored in a dedicated history table, allowing you to inspect how a record changed over time without requiring additional audit logic in your controllers or services.

Temporal ORM is designed to make historical data easy to work with while keeping the normal model workflow unchanged. Once enabled, Doppar handles the lifecycle hooks and history capture automatically, recording the state of each record together with metadata such as the action performed, the exact timestamp of the change, the columns that were modified, and optionally the authenticated user responsible for the change.

The recorded history can then be accessed through Doppar’s time-travel API. You can retrieve a record as it existed at a specific point in time, inspect its complete chronological history, compare two historical states, or create a temporary model instance representing an earlier state. When a historical state needs to become the current state again, Doppar also provides a restore operation that persists the selected state back to the database while recording the restoration as a new history entry.

Temporal ORM is particularly useful when your application needs reliable audit trails, historical reporting, debugging capabilities, compliance records, or the ability to undo changes. It works across Doppar’s supported database drivers and automatically adapts the history table structure and timestamp representation to the configured database.
...
The `migrate:temporal` command scans your `src/Models` directory, finds every model marked with `#[Temporal]`, and creates the corresponding history table.
...
| `--path=` | Path to scan for models (defaults to `src/Models`) |
...
```bash
php pool migrate:temporal --path=src/Domain/Models
```
