...
---
  ...
  - [Routing with Route Facade](#routing-with-route-facades)
  - [Route Helper Method](#route-helper-method)

## Routing
### Introduction
Doppar’s routing system provides a clean and expressive way to define your application’s URL structure and map incoming requests to the appropriate controller actions or closures. Routes can be defined using the **`Route`** facade or through the **`#[Route]`** attribute.

The routing engine includes powerful features such as route grouping and prefixes, named routes, request throttling, middleware assignment, and RESTful resource routing. Whether you’re building a lightweight API or a full-scale web application, Doppar’s routing system helps keep your application’s routes organized, consistent, maintainable, and easy to scale.
...
Basic usage example:
```php
use Phaseolies\Support\Router\Attributes\Mapper;
...
```php
use Phaseolies\Database\Entity\Attributes\Model;
...
Using the Route facade, you can quickly associate a URL with a specific piece of application logic. This makes route definitions easy to read, understand, and maintain, especially for smaller applications, APIs, and simple endpoints.

As your application grows, Doppar’s routing system also provides additional capabilities such as HTTP method-specific routes, named routes, route groups, middleware, prefixes, throttling, and resource routing—allowing you to start simple and scale your routing structure as needed.
...
All Doppar routes are defined in your route files, located within the `runtime/routes/*` directory. These files are automatically loaded by the framework. The `runtime/routes/web.php` file is dedicated to defining routes for your web interface and is assigned the web middleware group, which enables essential features like session management and CSRF protection.

For most Doppar applications, you will start by defining routes inside the `runtime/routes/web.php` file. Any route declared in this file can be accessed through its corresponding URL in the browser. For example, the following route can be accessed by visiting `http://example.com/user` in your browser:
...
## API Routes
Doppar provides separete api routes file localted in `runtime/routes/api.php`.
...
Route::post('/', fn (Request $request) => response(['success' => true]));
```
...
To apply middleware, use [attribute-based middleware](https://doppar.com/versions/4.x/middleware#attribute-based-middleware-loading) calling directly in your bundle controller.
