...
All configuration files for the Doppar framework are located in the `runtime/config` directory. These files define various options and settings that control how different parts of your application behave. Each configuration file is self-documented, so you are encouraged to explore and familiarize yourself with the available options.
...
Access the config value using the global helper
```php
$value = config('app.name');
```

Pass the second argument as a default value if the key doesn't exist
```php
$value = config('app.name', 'Doppar');
```

Access a config value using the facade, this will give the same output lie helper function.
```php
<?php
...
During runtime, you may also set configuration values dynamically using the Config facade or the global `config()` helper function. This can be useful in situations where you need to override or define configuration values on the fly—such as in testing, bootstrapping, or launchers.
...
> **Note:** Doppar caches the configuration data loaded from `env.toml`.
