...
If not specified, uses the default connection from `runtime/config/database.php`.
...
In your `runtime/config/database.php`, define a new connection named mysql_second:
```php
'mysql_second' => [
    'driver' => 'mysql',
...
Make sure your `env.toml` file contains the appropriate credentials:
```toml
