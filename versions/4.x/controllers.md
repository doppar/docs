...
## Create Controller
To quickly generate a new controller, you may run the `make:controller` Pool command. By default, all of the controllers for your application are stored in the `src/Http/Controllers` directory
...
```
This command will generate a controller in the `src/Http/Controllers/API` directory with all the typical bundle methods pre-defined except `create` and `edit` method.
