...
```
This command will place a new `IpBasedUserBlocked` class within your `src/Http/Middleware` directory. In this middleware, we will only allow access to the route if the supplied ip input matches a specified ip. Otherwise, we will redirect the users back to the /home URI:
```php
<?php
