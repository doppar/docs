...
## Relationships
### Introduction
Doppar Entity provides a powerful relationship system for defining and working with connections between related database records. Instead of manually managing foreign keys and separate queries, relationships allow your models to describe how their data is connected and provide an expressive interface for retrieving and querying related records.

Relationships are defined as methods on your Entity models and can represent common database associations such as one-to-one, one-to-many, and many-to-many relationships. Once defined, these relationships can be accessed through dynamic relationship properties, while the relationship methods themselves can be used as query builders to apply additional conditions, sorting, and other query constraints.

Doppar also provides flexible tools for efficiently loading and querying related data. With embed(), you can eager load relationships and nested relationships while selecting only the fields you need, applying conditions, and avoiding unnecessary database queries. Relationship existence and absence can also be used as query constraints, allowing you to retrieve records based on whether matching related records exist.

For applications that need relationship statistics, embedCount() provides a convenient way to retrieve counts of related records without loading the complete relationships. These counts can be filtered with conditions and combined with relationship loading, giving you precise control over how related data is retrieved.

In the following sections, we’ll explore how to define one-to-one, one-to-many, and many-to-many relationships, work with inverse relationships and pivot data, load nested relationships efficiently, apply relationship existence constraints, and retrieve counts of related records using Doppar Entity ORM.
