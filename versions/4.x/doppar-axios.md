...
Doppar Axios is a modern, feature-rich HTTP client for PHP, inspired by the simplicity and developer-friendly API of JavaScript clients like Axios. Built on top of Symfony HttpClient, it provides a fluent and expressive interface for making HTTP requests while offering the flexibility and reliability needed for real-world applications.

Unlike components that are tightly coupled to Doppar’s internal framework core, Doppar Axios is completely standalone and can be used in any modern PHP application. It supports common HTTP operations such as `GET`, `POST`, `PUT`, `PATCH`, and `DELETE`, while also providing convenient features for query parameters, custom headers, authentication, form data, redirects, SSL verification, and response handling.

Doppar Axios is designed for both simple and advanced HTTP workflows. You can create reusable, scoped clients with shared configuration, execute multiple requests asynchronously, automatically retry failed requests, configure request timeouts, attach custom middleware, and handle responses through convenient status and content helpers. It also provides streamed file downloads with progress tracking and multipart file uploads for applications that need to transfer large or complex payloads efficiently.

The client follows a fluent and composable design, allowing request configuration to be built through chainable methods without mutating previously configured client instances. This makes it easy to create shared HTTP clients for external APIs, microservices, authentication services, file servers, or other integrations while keeping configuration organized and reusable.
