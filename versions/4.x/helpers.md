...
### env()
The `env()` function in Doppar is used to retrieve environment variables from the application's configuration. It allows you to define environment-specific settings in an `env.toml` file and access them throughout your application, keeping their real type — a TOML boolean or number comes back as an actual `bool`/`int`, not a string. If the specified variable is not found, you can provide a default value as a fallback.
```php
...
### template_path()
The `template_path()` function is a helper that retrieves the application's template path, with an optional ability to append a specific subdirectory or file path. This function is particularly useful when dealing with application templates such as views, language files, and configuration files that are stored in the templates directory.
```php
// Get the base template path
$templatePath = template_path();

// Get the full resources path with a specific subdirectory or file
$fullPath = template_path('views/layouts/app.Odo.php');
```
...
### schema_path()
The `schema_path()` function is a helper designed to retrieve the application's schema path, with the ability to append a specific subdirectory or file path if necessary. This function is particularly useful for accessing the schema configuration, migration files, and other database-related files that are stored within the schema directory.
```php
// Get the base schema path
$schemaPath = schema_path();

// Get the full database path with a specific subdirectory or file
$fullPath = schema_path('migrations/');
```
### enqueue()
...
See more example from [collection](https://doppar.com/versions/4.x/collections)
