...
Cache::clear();
```
This will completely clear the cache.
...
```php
$users = Cache::stash('users', 60, fn () => User::all());
```
Parameters:
...
Finally returns the cached value.
...
```php
$users = Cache::stashForever('users', fn () => User::all());
```
...
$result = Cache::stashWhen(
    key: 'user_' . $userId,
    callback: fn() => User::query()->newest()->get(),
    condition: $shouldCache,
    ttl: 3600
);
```
This is ideal for scenarios where you want caching to occur only under certain conditions, giving you fine-grained control over performance and freshness.
